...
## 2026-07-11: VIGÍA MVP — Command Loop Pattern over defineTool
**By:** Trinity (Full-Stack Developer)  
**Tier:** T2 (Implementation)  
**Status:** ✅ MVP BUILT & VALIDATED

### Decision
Built VIGÍA MVP at `poc/vigia/` using a **command loop pattern** instead of `defineTool()`.

### Key Finding: defineTool is Broken for Custom Tools
The Copilot SDK v0.2.x `defineTool()` API does NOT work reliably when you want the agent to use ONLY your custom tools. The agent consistently prefers its built-in tools (`powershell`, `report_intent`, etc.) over any custom tools, even when the system prompt explicitly says to use custom tools.

**Evidence:** When given `navigate` and `screenshot` custom tools and told to "navigate to a URL", the agent used its built-in `powershell` tool with `Invoke-WebRequest` instead.

### Solution: Command Loop Pattern
Instead of `defineTool()`, the agent is instructed to emit structured JSON commands:
```json
{"commands": [
...
```
The script:
1. Sends mission prompt to agent
2. Collects the text response (with JSON blocks)
3. Parses JSON commands
4. Executes them via Playwright
5. Sends results back as next message
6. Loops until agent emits `{"action": "done"}`

**Advantages over defineTool:**
- No competition with built-in tools
- Full control over execution
- Debuggable (you can see exactly what the agent wants to do)
- Batching: agent can emit multiple commands per turn

### Deliverables
- `poc/vigia/vigia.js` — Main script with command loop
- `poc/vigia/tools/browser.js` — Playwright wrapper (9 actions)
- `poc/vigia/tools/reporter.js` — Issue tracking + Markdown report generation
- `poc/vigia/README.md` — Documentation

### Validation
Ran against `https://example.com` successfully:
- 5 turns, 21 actions, 3 issues found
- Screenshots captured (desktop + mobile)
- Markdown report generated with full action log
- Total time: ~51 seconds

### Note: CityPulseLabs URL is 404
`https://citypulselabs.azurestaticapps.net` returns 404. The MVP defaults to this URL but works with any `--url` parameter.

### Next Steps
- Fix CityPulseLabs deployment (Tank)
- Run VIGÍA against CityPulseLabs for real findings
- Consider Vercel AI SDK port (tool() + maxSteps avoids built-in competition entirely)
