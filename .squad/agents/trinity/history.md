...
- **VIGÍA v0.6.0 — Run Comparison + JSON Export (#166):** Added `--compare report1.json report2.json` CLI flag and JSON data export. `reporter.js` now writes `vigia-data-{timestamp}.json` alongside every markdown report — includes flat `issues[]` array with stable fingerprints (title+severity hash), plus structured `sessions[]` for multi-URL. New `lib/compare.js` module: `loadReport()` normalizes flat/structured formats, `compareReports(A, B, prev?)` categorizes issues as new/resolved/persistent/regression, `formatComparisonOutput()` produces terminal-friendly diff. Regression detection requires 3rd report (prev) to confirm disappeared-then-returned pattern. 203 tests green (48 from Switch TDD + Trinity impl). Key: fingerprint uses `title|severity` → djb2 hash for stable cross-run matching.
